...
#    print mini_line
    ...
    if mini_vol > 0:
        ...
        all_per = 0
    # 0 price, 1 per, 2 up_down_per, 3 direction, 4 result, 5 all_per, 6 eupdown, 7 volupdown, 8 mini_price_updown
    market_info = [mini_price, mini_per, up_down_per, 0, 0, all_per, eupdown, volupdown, mini_price_updown]
    ...
    walk_abs = abs(mini_walk)
    
    direction = 0
    tmp_direction = 0
#    if walk_abs > 0 and tmp3 > 0 and tmp > 0 and tmp5 > 0 and tmp2 > 0:
    if walk_abs > 1 and tmp > 0:
        g_pool = []
        tmp_direction = mini_per / abs(mini_per)
#    elif walk_abs < 7 and tmp4 > 0 and tmp3 > 0 and tmp > 0 and tmp5 > 0 and tmp2 > 0:
#        tmp_direction = -mini_per / abs(mini_per)
##    print market_info
##    print tmp, mini_per, up_down_per
#    if g_maket_info_prev:
#        diff1 = market_info[1] - g_maket_info_prev[1]
#        diff2 = market_info[2] - g_maket_info_prev[2]
#        diff3 = market_info[5] - g_maket_info_prev[5]
#        diff4 = market_info[6] - g_maket_info_prev[6]
#        diff5 = market_info[7] - g_maket_info_prev[7]
#        diff6 = market_info[8] - g_maket_info_prev[8]
#        tmp = diff1 * diff2
#        tmp2 = diff1 * diff3
#        tmp3 = diff1 * diff4
#        tmp4 = diff1 * diff5
#        tmp5 = diff1 * diff6
#        if walk_abs > 3 and tmp4 > 0 and tmp3 > 0 and tmp > 0 and tmp5 > 0 and tmp2 > 0:
##        if walk_abs > 0 and tmp5 > 0:
#    #    if tmp6 > 0:
#    #        print tmp
#            tmp_direction2 = diff1 / abs(diff1)
#            if tmp_direction * tmp_direction2 > 0:
